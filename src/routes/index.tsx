...
// ── Two circles diagram — ONE instance, lives in the fixed overlay ─────────
...
function Index() {
  const [mode, setMode]         = useState<"hero" | "split">("hero");
  ...
  // Scroll → trigger split at 55% of hero height
  useEffect(() => {
    if (mode !== "hero") return;
    const handler = () => { if (window.scrollY > window.innerHeight * 0.55) trigger(); };
  ...
  const trigger = useCallback((sectionId?: string) => {
    if (mode !== "hero") { if (sectionId) scrollToSection(sectionId); return; }
    setMode("split");
    if (sectionId) setTimeout(() => scrollToSection(sectionId), 900);
  ...
  return (
    <div className="relative">
      ...
      {/* Dear footer: always fixed behind */}
      <div className="fixed inset-0 z-0 bg-neutral-950"><DearFooter /></div>

      {/* ─────────────────────────────────────────────────────────────────
          SINGLE diagram overlay.
          Width animates: 100% (hero, centered) → 38% (split, left panel).
          The diagram stays centered inside the container at all times,
          so it visually slides from screen-center to left-panel-center.
      ───────────────────────────────────────────────────────────────── */}
      <div style={{
        position: "fixed",
        top: NAV_HEIGHT,
        ...
        width: isSplit ? LEFT_W : "100%",
        backgroundColor: isSplit ? "#fafafa" : "transparent",
        borderRight: isSplit ? "1px solid #f0f0f0" : "none",
        transition: `width 0.85s ${ease}, background-color 0.5s ease`,
        zIndex: 30,
        display: "flex",
        flexDirection: "column",
        alignItems: "center",
        justifyContent: "center",
        padding: isSplit ? "2.5rem" : "0 2rem",
        overflow: "hidden",
      }}>

        {/* Hero header — fades out when split */}
        <div style={{
          opacity: isSplit ? 0 : 1,
          transform: isSplit ? "translateY(-8px)" : "none",
          ...
          <p style={{ fontSize: 14, color: "#737373", lineHeight: 1.6, maxWidth: 340, margin: "0 auto" }}>
            Every exchange runs through them. Somewhere in each one, something gets lost.
          </p>
        </div>

        {/* THE diagram — one component, never unmounted */}
        <div style={{
          width: "100%",
          maxWidth: isSplit ? "none" : 600,
          transition: `max-width 0.85s ${ease}`,
          flexShrink: 0,
        }}>
          <TwoCirclesDiagram
            activeSegment={isSplit ? activeSegment : null}
            onSegmentClick={(segId) => trigger(SEGMENT_TO_SECTION[segId])}
          />
        </div>
        ...
        {/* Split: description only (no nav) — fades in */}
        <div style={{
          opacity: isSplit ? 1 : 0,
          transition: `opacity 0.45s ${isSplit ? "0.55s" : "0s"} ease`,
          width: "100%",
          marginTop: "2rem",
      ...
      <div className="relative z-10 bg-background" style={{ boxShadow: "0 0 80px 20px rgba(0,0,0,0.18)" }}>
        ...
        {/* Hero placeholder — collapses when split triggers */}
        <div style={{
          height: isSplit ? 0 : `calc(100vh - ${NAV_HEIGHT}px)`,
          overflow: "hidden",
          background: "#fafafa",
        ...
        <div className="flex">
          ...
          <div className="flex-1 min-w-0" style={{
            opacity: isSplit ? 1 : 0,
            transform: isSplit ? "none" : "translateX(20px)",
            transition: `opacity 0.55s ${isSplit ? "0.45s" : "0s"} ease, transform 0.55s ${isSplit ? "0.45s" : "0s"} ease`,
          }}>
